...
> [!NOTE]
> **Before or after deploying to Vercel:** In your Vercel Dashboard under **Project Settings ➔ Environment Variables**, add an environment variable named `NEXT_PUBLIC_API_URL` set to your live Railway backend URL (e.g., `https://rag-chatbot-production.up.railway.app`). If unset, `next.config.ts` will automatically fall back to the default production Railway API domain.
