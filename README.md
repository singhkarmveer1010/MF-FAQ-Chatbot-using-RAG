...
[![LLM](https://img.shields.io/badge/LLM-Groq%20Llama--3.3--70b-purple.svg)](https://console.groq.com)
...
- **🛡️ PII Detection & Security:** Integrated regular expression guardrails intercept and block queries containing PAN cards, Aadhaar numbers, phone numbers, email addresses, or OTPs.
...
├── tests/                    # Automated unit and format compliance test suites
├── vectorstore/              # Persistent SQLite ChromaDB vector index
├── Dockerfile                # Container deployment configuration
├── pytest.ini                # Test suite settings
├── requirements.txt          # Python project dependencies
├── test_api.py               # API end-to-end integration test suite
...
Run the automated test suite to verify ingestion pipelines, chunking logic, refusal compliance, and API routes:
```bash
# Run all unit and integration tests
pytest

# Run API end-to-end integration tests explicitly
python test_api.py
```
...
1. **Single AMC Focus:** The corpus is scoped exclusively to **HDFC Mutual Fund** schemes. Queries regarding other asset management companies (e.g., SBI, ICICI, Nippon) will return no results or an unsupported scheme guidance.
...
5. **Deterministic Generation Lock:** LLM generation temperature is permanently locked at `0.0` to eliminate stochastic variations and ensure factual reproducibility.
